...
def html_page(title: str, body: str, root: str) -> str:
...
<footer>KAISHOWチャンネル SNS下書き — 社内用。<br>generated by stand-fm-scrape</footer>
...
def main() -> None:
    ...
    for idx, ep in enumerate(sorted(episodes, key=lambda e: e["date"], reverse=True)):
        ...
        if has_post:
            ...
            body.append('<div class="card"><p class="pending">投稿下書きは未生成です'
                        '（文字起こし完了後に generate_posts.py で生成）。</p></div>')
        ...
        # ---- index card ----
        status = "✅ 下書きあり" if has_post else ("📝 文字起こし済" if has_txt else "⏳ 処理待ち")
        cards.append(
